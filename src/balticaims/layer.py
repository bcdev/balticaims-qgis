...
class DataCubeLayer(QgsRasterLayer):
    ...
    def read_data(self, ds):
        ...
        band_names = list(self.ds.data_vars.keys())
        for band_idx in range(1, n_time_steps+1):
            ...
            if self._progress_bar:
                self._progress_bar.setValue(band_idx)
        del mem_ds
    ...
    def set_single_band_pseudo_color_table(self,
        ...
        if color_ramp_data is None:
            color_ramp_data = self.DEFAULT_COLOR_RAMP_DATA
        color_ramp_item_list = self.interpolate_color_ramp(color_ramp_data, color_ramp_min, color_ramp_max)
        shader_fn = QgsColorRampShader(color_ramp_min or self.DEFAULT_COLOR_RAMP_MIN, color_ramp_max or self.DEFAULT_COLOR_RAMP_MAX)
    ...
    @staticmethod
    def interpolate_color_ramp(color_ramp_data, color_ramp_min: float=0, color_ramp_max: float=255):
        values = np.linspace(color_ramp_min, color_ramp_max, len(color_ramp_data))
        return [
            QgsColorRampShader.ColorRampItem(values[i], QColor(int(r * 255), int(g * 255), int(b * 255))) for i, (r,g,b) in enumerate(color_ramp_data)
        ]

    def set_time_range_per_band(self, time_index: pd.Series) -> bool:
        """
        Set the for each band (representing a single time
        step of a data cube) to range from an entry in the
        ...
        time_steps = [QDateTime.fromString(t.isoformat(), Qt.ISODate) for t in time_index]
        ranges = [QgsDateTimeRange(t,t) for t in time_steps]
        self.logger.info(f"{ranges=}")
